...
static int calc_image_info(void *conf)
{
	...
	if( info->src_type > 0)
	{
		...
		if(info->src_im != NULL)
		{
			...
			{
				if((info->max_width > info->src_width) && (info->max_width > ((float)info->src_width * 1.5)))
                		{
                   			 info->w_margin = 1;
                    			 info->src_width = info->max_width;
                    			 info->src_height = (float)info->src_h * ((float)info->max_width / info->src_w);
                   			 info->width = (float)info->src_w * 1.5;
                    			 info->height = (float)info->src_h * 1.5;
                    			 info->dst_x = (float)((float)(info->src_width - info->width)/2);
                    			 info->dst_y = (float)((float)(info->src_height - info->height)/2);
                		}
                		else
                		{
                    			info->width = info->max_width;
